
<script>
    import 'emoji-data-css/files/gl-16-emoji.css'
    import lodash_split from 'lodash/split'
export default {
    props:['text','list'],
    render(h){
        let src = 'data:image/png;base64,'

        console.log(lodash_split(this.text,''));
        console.log(this.list);
        let splitStr = lodash_split(this.text,'')
        splitStr = splitStr.map(item=>{

            let find  = this.list.find(itemFind=>itemFind.emoji===item)
            if(!find.emojiName) return

        let className="gl gl-16 gl-"+find.emojiName
            if (item.length>1) { item = (<img class={className}   src={src} alt={item}/>) }
            return item
        })
        return (


        <div id="contenteditable" contenteditable="true">{splitStr}</div>
        )
    }
}
</script>

<style lang="scss">

</style>