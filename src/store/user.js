...
export default {
    ...
    actions: {
        getLogin({ commit, dispatch }, user) {
            ...
            this._vm.$socket.query = `uuid=${user.uuid}&user=employee&socketGroupId=${user.owner_id}`
            this._vm.$socket.io.uri=user.socket_server

            this._vm.$socket.io.opts.query = `uuid=${user.uuid}&user=employee&socketGroupId=${user.owner_id}`
